...
export function requestActionLabel(kind: RequestTarget["kind"]) {
  ...
  return "Request this part / arrange pickup";
}
...
export default function YardRequestForm({
  ...
  const send = async (attempt: YardRequestAttempt) => {
    ...
    setError("");
    try {
      const result = await submitRequest(attempt);
      setReceipt(result);
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/55 p-0 sm:items-center sm:p-5" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setOpen(false); }}>
      <section ref={dialogRef} role="dialog" aria-modal="true" aria-label={title} onKeyDown={handleDialogKeyDown} className="max-h-[94vh] w-full overflow-y-auto rounded-t-3xl bg-white p-6 shadow-2xl sm:max-w-xl sm:rounded-3xl sm:p-8">
        ...
        </div> : <form className="mt-7 space-y-4" onSubmit={submit}>
          {target.kind === "donor_part" && <Field label="Part or component" name="component" required placeholder="Example: driver-side headlight" />}
          <Field label="Your name" name="customer_name" required autoComplete="name" data-initial-focus />
          <Field label="Email" name="customer_email" type="email" required autoComplete="email" />
          <Field label="Phone (optional)" name="customer_phone" type="tel" autoComplete="tel" />
          {target.kind !== "whole_car" && <Field label="Target vehicle (optional)" name="target_vehicle" placeholder="Year, make, model, trim or VIN" />}
          <label className="block text-sm font-semibold text-zinc-800">Notes (optional)<textarea name="message" rows={3} className="mt-1.5 w-full rounded-xl border border-zinc-300 px-3 py-2 font-normal focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-100" /></label>

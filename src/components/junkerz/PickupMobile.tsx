...
export default function PickupMobile(){
  ...
  const arrived=['arrived','photographed','signed','paid'].includes(detail.status);
  return <main className="min-h-screen bg-stone-50 text-slate-950">
    <div className="mx-auto max-w-lg px-5 pb-16">
      ...
      <div className="py-7">
        <span className={`inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold ${completed?'bg-emerald-100 text-emerald-900':'bg-slate-950 text-white'}`}>{completed?<Check size={14}/>:<Truck size={14}/>} {completed?'Pickup completed':enRoute?'On the way':arrived?'Arrived':detail.status.replace(/_/g,' ')}</span>
      ...
      {closed ? <section className="mb-6 rounded-xl border border-slate-200 bg-white p-5"><h2 className="font-semibold">{completed?'Already completed':'This pickup is closed'}</h2><p className="mt-2 text-sm text-slate-600">{completed?'This pickup was completed earlier. Opening this link shows the saved record; it does not complete another pickup.':'Ask the buyer for an updated assignment.'}</p>{detail.completed_at && <p className="mt-2 text-xs text-slate-500">Recorded completion: {dateTime(detail.completed_at)}</p>}</section> : <>
        ...
        <Step number="05" title="Seller signature">
        ...
        <Step number="06" title="Finish pickup">
          ...
          {!arrived && <p className="mb-3 text-sm text-amber-800">Confirm your arrival before completing pickup.</p>}
          {!detail.seller_signature && <p className="mb-3 text-sm text-amber-800">Save the seller’s signature before completing pickup.</p>}
          <button className={`${actionClass} !bg-brand-600 hover:!bg-brand-700 disabled:!bg-slate-200`} disabled={!identified || !!busy || !arrived || !detail.seller_signature || !collected} onClick={()=>void run('complete',()=>api.complete(driver.trim()),'Pickup completed. The buyer’s office has the record.')}><Check size={21}/>{busy==='complete'?'Completing…':'Pickup complete'}</button>
        </Step>
      </>}

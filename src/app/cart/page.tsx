...
export default async function CartPage() {
  ...
  const hasLive = Object.values(settings.sections).some((mode) => mode === "live") && !settingsUnavailable;
  return <main className="min-h-screen bg-zinc-50 text-zinc-900">
    <SiteHeader catalogSettings={settingsUnavailable ? null : settings} />
    {hasLive && <div className="bg-amber-100 px-5 py-3 text-center text-sm font-semibold text-amber-950">SANDBOX CART · No real orders or charges</div>}
